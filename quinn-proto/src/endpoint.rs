...
// ── TLS-offload accept API ────────────────────────────────────────────────────

/// Minimal `ConnectionIdGenerator` used when constructing a `Connection` without
/// the endpoint's generator on hand. Only `cid_len` and `cid_lifetime` are called
/// by `Connection::new`; `generate_cid` is never reached.
struct StaticCidParams {
    len: usize,
    lifetime: Option<Duration>,
}

impl ConnectionIdGenerator for StaticCidParams {
    fn generate_cid(&mut self) -> ConnectionId {
        unreachable!("StaticCidParams::generate_cid should never be called")
    }
...
/// Carries everything needed to construct a `Connection` and run the TLS Initial
/// handshake, without requiring `&mut Endpoint`.
///
/// Created by [`Endpoint::pre_accept`] on the engine thread. Pass to
/// [`PendingAccept::complete`] on any thread (e.g., an application async task) to
/// run TLS. Then pass the [`AcceptOutcome`] to [`Endpoint::finish_accept`] on the
/// engine thread to register the connection.
pub struct PendingAccept {
    ch: ConnectionHandle,
    incoming_idx: usize,
    // Fields forwarded to Connection::new
    version: u32,
    init_cid: ConnectionId,
    loc_cid: ConnectionId,
    rem_cid: ConnectionId,
    ...
    cid_gen_len: usize,
    cid_gen_lifetime: Option<Duration>,
    allow_mtud: bool,
    rng_seed: [u8; 32],
...
impl PendingAccept {
    ...
    /// The local IP used for this connection attempt
    pub fn local_ip(&self) -> Option<IpAddr> {
        self.addresses.local_ip
    }

    /// The `ConnectionHandle` reserved by [`Endpoint::pre_accept`].
    ///
    /// Use this to key into a pending-accept map on the engine side.
    pub fn ch(&self) -> ConnectionHandle {
        self.ch
    }

    /// Run the TLS Initial handshake. May be called on any thread.
    ///
    /// Constructs the `Connection`, drives the TLS state machine through the
    /// Initial level (generating the server's first flight), and returns an
    /// [`AcceptOutcome`] to be passed to [`Endpoint::finish_accept`].
    pub fn complete(self, now: Instant, buf: &mut Vec<u8>) -> AcceptOutcome {
        let tls = self
            .server_config
            .crypto
            .clone()
        ...
        let cid_params = StaticCidParams {
            len: self.cid_gen_len,
            lifetime: self.cid_gen_lifetime,
        };
        ...
        let mut conn = Connection::new(
            ...
            self.server_config.transport.clone(),
            self.init_cid,
            self.loc_cid,
            self.rem_cid,
            self.addresses.remote,
        ...
        match conn.handle_first_packet(
            ...
            Ok(()) => AcceptOutcome::Ready(AcceptReady {
                ch: self.ch,
                incoming_idx: self.incoming_idx,
                conn,
                init_cid: self.init_cid,
                loc_cid: self.loc_cid,
                addresses: self.addresses,
            }),
            Err(e) => AcceptOutcome::Failed(AcceptFailed {
                ch: self.ch,
                incoming_idx: self.incoming_idx,
                cause: e,
...
/// Result of [`PendingAccept::complete`].
pub enum AcceptOutcome {
    Ready(AcceptReady),
    Failed(AcceptFailed),
}

/// A successfully completed TLS handshake, ready to be registered by the engine.
...
/// A failed TLS handshake, needs engine-side cleanup.
pub struct AcceptFailed {
    ch: ConnectionHandle,
    incoming_idx: usize,
    /// The reason the handshake failed. Exposed for error propagation.
    pub cause: ConnectionError,
    version: u32,
    addresses: FourTuple,
    crypto: Keys,
...
impl Endpoint {
    /// Step 1 of the TLS-offload accept flow. Validates the `Incoming`, allocates
    /// a `ConnectionHandle` and connection IDs, and returns a [`PendingAccept`]
    /// that can be passed to any thread for TLS processing.
    ///
    /// Incoming packets for the new connection continue to be buffered inside the
    /// endpoint until [`finish_accept`][Self::finish_accept] is called.
    // AcceptError cannot be made smaller without semver breakage
    #[allow(clippy::result_large_err)]
    pub fn pre_accept(
        &mut self,
        ...
        now: Instant,
        buf: &mut Vec<u8>,
    ) -> Result<PendingAccept, AcceptError> {
        let remote_address_validated = incoming.remote_address_validated();
        incoming.improper_drop_warner.dismiss();
        ...
        // Allocate connection slot and CIDs (all engine-thread work; no TLS yet)
        let ch = ConnectionHandle(self.connections.vacant_key());
        let loc_cid = self.new_cid(ch);
        let mut params = TransportParameters::new(
            &server_config.transport,
        ...
        // Reserve the connection slot so subsequent pre_accept calls get distinct handles.
        // The slot will be populated with full routing info in finish_accept.
        let mut cids_issued = 0;
        let mut loc_cids = FxHashMap::default();
        loc_cids.insert(cids_issued, loc_cid);
        cids_issued += 1;
        if let Some(cid) = pref_addr_cid {
            loc_cids.insert(cids_issued, cid);
            // cids_issued += 1;  (would be used if we needed the count here)
        }
        let id = self.connections.insert(ConnectionMeta {
            init_cid: dst_cid,
            cids_issued,
        ...
        // Note: incoming_buffers entry is kept alive; packets that arrive while
        // the app thread is running TLS will continue to be buffered there.
        ...
        Ok(PendingAccept {
            ...
            cid_gen_len: self.local_cid_generator.cid_len(),
            cid_gen_lifetime: self.local_cid_generator.cid_lifetime(),
    ...
    /// Step 3 of the TLS-offload accept flow. Called on the engine thread after
    /// the app thread has called [`PendingAccept::complete`].
    ///
    /// On success: registers the connection in the endpoint's routing tables,
    /// drains buffered packets into the connection, and returns `(handle, conn)`.
    ///
    /// On failure: cleans up the reserved slot and returns an [`AcceptError`].
    // AcceptError cannot be made smaller without semver breakage
    #[allow(clippy::result_large_err)]
    pub fn finish_accept(
        ...
        match outcome {
            AcceptOutcome::Ready(ready) => {
                let ch = ready.ch;
                // Drain packets that arrived while TLS was running on the app thread.
                let incoming_buffer = self.incoming_buffers.remove(ready.incoming_idx);
                self.all_incoming_buffers_total_bytes -= incoming_buffer.total_bytes;

                // Register in routing tables.
                self.index.insert_conn(ready.addresses, ready.loc_cid, ch, Side::Server);
                self.index.insert_initial(ready.init_cid, ch);

                let mut conn = ready.conn;
                for event in incoming_buffer.datagrams {
                    conn.handle_event(ConnectionEvent(ConnectionEventInner::Datagram(event)));
                }

                trace!(id = ch.0, icid = %ready.init_cid, "new connection (deferred TLS accept)");
                Ok((ch, conn))
            }
            AcceptOutcome::Failed(failed) => {
                // Remove buffered packets and connection slot reserved in pre_accept.
                let incoming_buffer = self.incoming_buffers.remove(failed.incoming_idx);
                self.all_incoming_buffers_total_bytes -= incoming_buffer.total_bytes;
                // handle_event(Drained) removes from connections slab and from index
                // (including the connection_ids_initial entry set by handle_first_packet).
                self.handle_event(
                    failed.ch,
                    EndpointEvent(EndpointEventInner::Drained),
                );

                let response = match &failed.cause {
                ...
                debug!("deferred handshake failed: {}", failed.cause);
                Err(AcceptError {
                    cause: failed.cause,
                    response,
                })
