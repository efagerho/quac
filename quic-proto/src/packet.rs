...
#[derive(Debug)]
pub struct PartialDecode<B: RecvBuf = BytesMut> {
    plain_header: ProtectedHeader,
    buf: io::Cursor<B>,
}

#[allow(clippy::len_without_is_empty)]
impl<B: RecvBuf> PartialDecode<B> {
    ...
    ) -> Result<(Self, Option<B>), PacketDecodeError> {
    ...
    pub(crate) fn finish(
    ...
    ) -> Result<Packet<B>, PacketDecodeError> {
        use ProtectedHeader::*;
        let Self {
            plain_header,
            mut buf,
        ...
        }) = plain_header
        {
            ...
            // Copy header bytes (~30 B) eagerly; avoids making Header generic over B.
            let header_prefix = bytes.split_to(header_len);
            let header_data = Bytes::copy_from_slice(header_prefix.as_ref());
            drop(header_prefix);
        ...
        let header = match plain_header {
            ...
            Short { spin, dst_cid, .. } => {
                let number = Self::decrypt_header(&mut buf, header_crypto.unwrap())?;
                let key_phase = buf.get_ref().as_ref()[0] & KEY_PHASE_BIT != 0;
                Header::Short {
                    spin,
                    key_phase,
                    dst_cid,
    ...
    fn decrypt_header(
        ...
        header_crypto.decrypt(pn_offset, buf.get_mut().as_mut());

        let len = PacketNumber::decode_len(buf.get_ref().as_ref()[0]);
        PacketNumber::decode(len, buf)
    }
}

pub(crate) struct Packet<B: RecvBuf = BytesMut> {
    pub(crate) header: Header,
    pub(crate) header_data: Bytes,
    pub(crate) payload: B,
}

impl<B: RecvBuf> Packet<B> {
...
pub(crate) struct InitialPacket<B: RecvBuf = BytesMut> {
    pub(crate) header: InitialHeader,
    pub(crate) header_data: Bytes,
    pub(crate) payload: B,
}

impl<B: RecvBuf> From<InitialPacket<B>> for Packet<B> {
    fn from(x: InitialPacket<B>) -> Self {
        Self {
...
mod tests {
    ...
    fn header_encoding() {
        ...
        let decode: PartialDecode = PartialDecode::new(
            buf.as_slice().into(),
        ...
        let plain_len = server
            .packet
            .remote
            .decrypt(0, &packet.header_data, packet.payload.as_mut())
            .unwrap();
        packet.payload.truncate(plain_len);
        assert_eq!(&packet.payload[..], &[0u8; 16]);
        match packet.header {
