...
fn version_negotiate_server() {
    ...
    let mut server: Endpoint = Endpoint::new(Default::default(), Some(Arc::new(server_config())), true);
    let now = Instant::now();
...
fn version_negotiate_client() {
    ...
    let mut client: Endpoint = Endpoint::new(
    ...
    if let Some(DatagramEvent::ForConnection(_, event)) = opt_event {
        client_ch.handle_datagram(event);
    }
    assert_matches!(
        client_ch.poll(),
