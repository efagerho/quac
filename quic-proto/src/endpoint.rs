...
pub struct Endpoint<B: RecvBuf = BytesMut> {
    rng: StdRng,
    ...
    incoming_buffers: Slab<IncomingBuffer<B>>,
    all_incoming_buffers_total_bytes: u64,
}

impl<B: RecvBuf> Endpoint<B> {
    ...
    pub fn handle(
        ...
        data: B,
        buf: &mut Vec<u8>,
    ) -> Option<DatagramEvent<B>> {
        #[cfg(debug_assertions)]
        if std::env::var_os("QUAC_TRACE_PROTO").is_some() {
            trace!(
                remote = %remote,
                len = data.len(),
                ecn = ?ecn,
                "QUAC_TRACE_PROTO endpoint.handle"
            );
        }
        ...
        if let Some(route_to) = self.index.get(&addresses, &event.first_decode) {
            ...
            match route_to {
                ...
                RouteDatagramTo::Connection(ch) => {
                    Some(DatagramEvent::ForConnection(ch, event))
                }
            }
    ...
    fn handle_first_packet(
        &mut self,
        datagram_len: usize,
        event: DatagramConnectionEvent<B>,
        addresses: FourTuple,
        buf: &mut Vec<u8>,
    ) -> Option<DatagramEvent<B>> {
        let dst_cid = event.first_decode.dst_cid();
    ...
    pub fn accept(
        ...
        match incoming.crypto.packet.remote.decrypt(
            packet_number,
            &incoming.packet.header_data,
            incoming.packet.payload.as_mut(),
        ) {
            Ok(plain_len) => incoming.packet.payload.truncate(plain_len),
            Err(_) => {
    ...
    pub fn refuse(&mut self, incoming: Incoming<B>, buf: &mut Vec<u8>) -> Transmit {
    ...
    pub fn ignore(&mut self, incoming: Incoming<B>) {
        self.clean_up_incoming(&incoming);
        incoming.improper_drop_warner.dismiss();
    }

    /// Clean up endpoint data structures associated with an `Incoming`.
    fn clean_up_incoming(&mut self, incoming: &Incoming<B>) {
    ...
    /// Length of local connection IDs generated for this endpoint.
...
struct IncomingBuffer<B: RecvBuf = BytesMut> {
    datagrams: Vec<DatagramConnectionEvent<B>>,
    total_bytes: u64,
}

impl<B: RecvBuf> Default for IncomingBuffer<B> {
    fn default() -> Self {
        Self {
            datagrams: Vec::new(),
            total_bytes: 0,
...
impl ConnectionIndex {
    ...
    fn get<B: RecvBuf>(&self, addresses: &FourTuple, datagram: &PartialDecode<B>) -> Option<RouteDatagramTo> {
...
pub enum DatagramEvent<B: RecvBuf = BytesMut> {
    /// The datagram is routed to an existing connection.
    ForConnection(ConnectionHandle, DatagramConnectionEvent<B>),
    /// The datagram may result in starting a new `Connection`
    NewConnection(Incoming<B>),
    /// Response generated directly by the endpoint
    Response(Transmit),
}

/// An incoming connection for which the server has not yet begun its part of the handshake.
pub struct Incoming<B: RecvBuf = BytesMut> {
    received_at: Instant,
    addresses: FourTuple,
    ecn: Option<EcnCodepoint>,
    packet: InitialPacket<B>,
    rest: Option<B>,
    crypto: Keys,
    token: IncomingToken,
    incoming_idx: usize,
...
impl<B: RecvBuf> fmt::Debug for Incoming<B> {
...
pub struct RetryError<B: RecvBuf = BytesMut>(Box<Incoming<B>>);

impl<B: RecvBuf> RetryError<B> {
    /// Get the [`Incoming`]
    pub fn into_incoming(self) -> Incoming<B> {
        *self.0
    }
}
