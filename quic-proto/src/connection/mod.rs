...
use crate::{
    ...
    MIN_INITIAL_SIZE, RecvBuf, Side, StreamId, TIMER_GRANULARITY, TokenStore, Transmit,
...
impl Connection {
    ...
    /// Process a received datagram routed to this connection.
    pub fn handle_datagram<B: RecvBuf>(&mut self, event: DatagramConnectionEvent<B>) {
        let DatagramConnectionEvent {
            now,
            remote,
            ecn,
            first_decode,
    ...
    /// efficient.
    pub(crate) fn handle_first_packet<B: RecvBuf>(
        &mut self,
        now: Instant,
        remote: SocketAddr,
        ecn: Option<EcnCodepoint>,
    ...
    fn handle_decode<B: RecvBuf>(
        &mut self,
        now: Instant,
        remote: SocketAddr,
        ecn: Option<EcnCodepoint>,
        partial_decode: PartialDecode<B>,
    ) {
    ...
    fn handle_packet<B: RecvBuf>(
        &mut self,
        now: Instant,
        remote: SocketAddr,
        ecn: Option<EcnCodepoint>,
        packet: Option<Packet<B>>,
    ...
    fn process_decrypted_packet<B: RecvBuf>(
        &mut self,
        now: Instant,
        remote: SocketAddr,
        number: Option<u64>,
        ...
        let state = match self.state {
            ...
            State::Closed(_) => {
                let payload = Bytes::copy_from_slice(packet.payload.as_ref());
        ...
        match packet.header {
            Header::Retry {
                ...
                *token = Bytes::copy_from_slice(&packet.payload.as_ref()[..token_len]);
    ...
    fn process_early_payload<B: RecvBuf>(
        &mut self,
        now: Instant,
        packet: Packet<B>,
    ) -> Result<(), TransportError> {
    ...
    fn process_payload<B: RecvBuf>(
        ...
        // Pool buffer boundary: one copy to BytesMut, then drop pool buffer.
        let payload_bytes = BytesMut::from(packet.payload.as_ref());
        drop(packet.payload);
    ...
    pub(crate) fn decode_packet(
        &self,
        event: &DatagramConnectionEvent<bytes::BytesMut>,
    ) -> Option<Vec<u8>> {
        if event.remaining.is_some() {
