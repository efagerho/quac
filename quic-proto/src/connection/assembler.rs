...
pub(super) struct Assembler {
    state: State,
    /// Inline slot for the common case: in-order delivery in ordered mode.
    /// Holds at most one buffer whose offset equals `bytes_read`. Bypasses
    /// the heap entirely when set, saving a BinaryHeap push + sift-up.
    next: Option<Buffer>,
...
impl Assembler {
    ...
    pub(super) fn ensure_ordering(&mut self, ordered: bool) -> Result<(), IllegalOrderedRead> {
        if ordered && !self.state.is_ordered() {
            ...
            // Flush inline slot to heap before transitioning
            if let Some(slot) = self.next.take() {
                self.data.push(slot);
            }
    ...
    pub(super) fn read(&mut self, max_length: usize, ordered: bool) -> Option<Chunk> {
        // Fast path: serve from inline slot — avoids heap peek overhead for in-order streams
        if ordered {
            if let Some(ref mut slot) = self.next {
                return Some(if max_length < slot.bytes.len() {
                    self.bytes_read += max_length as u64;
                    let offset = slot.offset;
                    slot.offset += max_length as u64;
                    self.buffered -= max_length;
                    Chunk::new(offset, slot.bytes.split_to(max_length))
                } else {
                    self.bytes_read += slot.bytes.len() as u64;
                    self.buffered -= slot.bytes.len();
                    self.allocated -= slot.allocation_size;
                    let slot = self.next.take().unwrap();
                    Chunk::new(slot.offset, slot.bytes)
                });
            }
        }
    ...
    fn defragment(&mut self) {
        // Flush inline slot to heap so defragment sees all buffered data
        if let Some(slot) = self.next.take() {
            self.data.push(slot);
        }
    ...
    pub(super) fn insert(&mut self, mut offset: u64, mut bytes: Bytes, allocation_size: usize) {
        ...
        // Fast path: in-order delivery with no pending out-of-order data.
        // Storing in the inline slot avoids a BinaryHeap push + sift-up.
        if matches!(self.state, State::Ordered)
            && self.next.is_none()
            && self.data.is_empty()
            && offset == self.bytes_read
        {
            self.next = Some(buffer);
            return;
        }
        // Off fast path — flush inline slot to heap first so ordering is preserved.
        if let Some(slot) = self.next.take() {
            self.data.push(slot);
        }
        self.data.push(buffer);
...
mod test {
    ...
    fn ordered_eager_discard() {
        let mut x = Assembler::new();
        x.insert(0, Bytes::from_static(b"abc"), 3);
        // In-order: stored in inline slot, not the heap
        ...
        // Duplicate (offset 0..2 already consumed) — discarded before reaching inline slot
        x.insert(0, Bytes::from_static(b"ab"), 2);
        assert!(x.next.is_none());
        assert_eq!(x.data.len(), 0);
        // Partial overlap: trimmed to "d" at offset 3, stored in inline slot
