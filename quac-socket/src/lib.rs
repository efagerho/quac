...
#[cfg(debug_assertions)]
#[inline]
fn debug_socket_recv_enabled() -> bool {
...
#[cfg(debug_assertions)]
#[inline]
fn socket_recv_log_enabled() -> bool {
...
#[cfg(debug_assertions)]
#[inline]
fn socket_send_log_enabled() -> bool {
    std::env::var_os("QUAC_SOCKET_SEND_LOG").is_some()
}

#[cfg(debug_assertions)]
fn hex_prefix_from_transmit(
...
#[cfg(debug_assertions)]
fn log_socket_send_datagram(t: &Transmit<ScatterGather<OsBuf>>) {
...
#[cfg(not(debug_assertions))]
#[inline(always)]
fn log_socket_send_datagram(_t: &Transmit<ScatterGather<OsBuf>>) {}
...
// Per-engine-thread free-list for send-side OsBuf allocations.
// All OsBuf drops happen on the engine thread (inline after sendmmsg or in
// drain_completions), so thread-local storage gives us zero-cost recycling
// with no atomics.
thread_local! {
...
/// Mutable heap buffer used by the pool for outgoing packets.
pub struct OsBufMut(Vec<u8>);
...
impl BufferPool for OsPool {
    ...
    fn alloc(&self, capacity: usize, count: usize, bufs: &mut Vec<OsBufMut>) -> usize {
        for _ in 0..count {
            ...
            let data = match v {
                Some(mut recycled) => {
                    // resize is a no-op when len == capacity (common case).
...
#[repr(align(64))]
struct RecvBuf([u8; MAX_DATAGRAM]);

fn alloc_recv_buf() -> Box<RecvBuf> {
...
impl PacketSocket for OsSocket {
    ...
    fn recv(
        ...
        for i in 0..received {
            ...
            let mut data = BytesMut::with_capacity(msg_len);
            data.extend_from_slice(&slot.buf.0[..msg_len]);
        ...
        while count < meta.len() {
            match self.socket.recv_from(&mut self.recv_buf.0) {
                Ok((len, src)) => {
                    ...
                    data.extend_from_slice(&self.recv_buf.0[..len]);
                    bufs.push(ScatterGather {
...
mod tests {
    ...
    fn send_one(sock: &mut OsSocket, dest: SocketAddr, payload: &[u8]) -> bool {
        let buf = super::OsBuf::from_slice(payload);
        let len = payload.len();
