#!/usr/bin/env bash
# Profile CPU of os_socket_bench in pong mode while blaster sends ping traffic.
#
# Blaster: 4 threads, 16-byte payload, batch-size 4, ping (send+recv) mode.
# Profiles only the pong server process.
#
# Usage:
#   ./scripts/profile_os_socket_bench.sh
#
# Env overrides:
#   PORT            UDP port (default: 4100)
...
PERF_DATA="${PERF_DATA:-$OUT_DIR/perf_os_socket_bench.data}"
SVG="${SVG:-$OUT_DIR/flamegraph_os_socket_bench.svg}"
...
echo "=== os_socket_bench pong profile ===" >&2
...
PONG_BIN="$REPO_ROOT/target/release/os_socket_bench"
...
echo "" >&2
echo "=== Top hotspots (os_socket_bench pong) ===" >&2
