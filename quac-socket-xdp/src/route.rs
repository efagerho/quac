...
impl RoutingTables {
    pub(crate) fn new(routes: Routes, neighbors: Neighbors, interfaces: Interfaces) -> Self {
        Self {
            routes,
            neighbors,
            interfaces,
    ...
    pub(crate) fn from_netlink(table: RouteTable) -> Result<Self, io::Error> {
        Ok(Self::new(
    ...
    pub(crate) fn upsert_route(&mut self, new_route: RouteEntry) -> bool {
        self.routes.upsert(new_route)
    }

    pub(crate) fn remove_route(&mut self, new_route: RouteEntry) -> bool {
        self.routes.remove(new_route)
    }

    pub(crate) fn upsert_neighbor(&mut self, new_neighbor: NeighborEntry) -> bool {
        self.neighbors.upsert(new_neighbor)
    }

    pub(crate) fn remove_neighbor(&mut self, ip: Ipv4Addr, if_index: u32) -> bool {
        self.neighbors.remove(ip, if_index)
    }

    pub(crate) fn upsert_interface(&mut self, new_interface: InterfaceInfo) -> bool {
        self.interfaces.upsert(new_interface)
    }

    pub(crate) fn remove_interface(&mut self, if_index: u32) -> bool {
...
impl Router {
    ...
    /// the populated router once the dump completes.
    pub fn empty() -> Self {
        Self {
            neighbors: Neighbors::new(Vec::new()),
            routes: Routes::new(RouteTable::Main, Vec::new()),
            ipv4_lpm: Ipv4Lpm::build(&[]),
            cached_default_route: None,
        }
    }

    pub(crate) fn from_tables(tables: RoutingTables) -> Result<Self, io::Error> {
    ...
    /// Default-route lookup. `pub(crate)` so the public Router API only
    /// exposes [`route_v4`](Self::route_v4); send-path callers always go
    /// through that and never need this directly.
    pub(crate) fn default(&self) -> Result<NextHop, RouteError> {
        if let Some(default_route) = &self.cached_default_route {
