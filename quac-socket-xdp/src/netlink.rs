...
impl NetlinkSocket {
    ...
    pub fn bind(groups: u32) -> Result<Self, io::Error> {
        ...
        // Ensure the kernel actually honored our SO_RCVBUF request. A silent
        // fall-back to the default (often 4 KiB on tight cgroups) can truncate
        // large netlink dumps without warning.
        let rc = unsafe {
            setsockopt(
                sock.as_raw_fd(),
                SOL_SOCKET,
